...
<pre>
화면속에 있는 아이템들은 실제로 같은 평면위에 놓여있지 않습니다. <br>
하지만 우리 화면에서는 마치 모든 오브젝트들이 평평한 나의 모니터 위에 놓여있는 것 처럼 보입니다. <br>
그렇기 때문에 마우스로 상호작용하는 게임의 경우 멀리 있는 물체와 가까이 있는 물체 모두 마우스를 갖다대는 것으로 선택할 수가 있습니다.<br>
이 내용은 ViewPort Point / Screen Point를 알면 쉽게 이해할 수 있는데 오늘은 "마우스로 물건을 선택했다"라는 것에 초점을 맞추고자 합니다.<br>
어떤 방법을 사용해야 마우스로 오브젝트를 선택할 수 있을까요?<br>
